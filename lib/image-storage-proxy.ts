...
export async function saveImage(blob: Blob, mimeType: string): Promise<string> {
  ...
  const resp = await chrome.runtime.sendMessage({ type: 'IMAGE_SAVE', buffer, mimeType });
  return resp.id as string;
}

export async function getImage(id: string): Promise<Blob | null> {
  const resp = await chrome.runtime.sendMessage({ type: 'IMAGE_GET', id });
...
export async function deleteImages(ids: string[]): Promise<void> {
  ...
  await chrome.runtime.sendMessage({ type: 'IMAGE_DELETE_MANY', ids });
}
